"""Extracción de series de inflación desde BANREP/SUAMECA.

Pipeline de 3 capas:

1. **SESIÓN**    — Warm-up HTTP para establecer cookies/sesión con SUAMECA.
2. **EXTRACCIÓN** — Consulta el API REST por cada serie (Inf_Goal,
   Inf_Rate, Core_Inf) y descarga los datos JSON.
3. **PARSING**   — Convierte epoch_ms → fecha mensual, une las 3 series
   en un DataFrame único y genera el formato largo estándar.

Formato de respuesta SUAMECA::

    [
      {
        "id": 15270,
        "nombre": "Inflación total",
        ...
        "data": [[epoch_ms, valor], [epoch_ms, valor], ...]
      }
    ]

Las fechas vienen como end-of-month (e.g. ``2026-02-28``);
se normalizan a inicio de mes (``2026-02-01``) para consistencia
con el resto del proyecto.
"""

from __future__ import annotations

import json
import logging
from datetime import date, datetime
from pathlib import Path

import pandas as pd
import requests

from src.config import (
    BANREP_INFLATION_CONFIG,
    BANREP_PROCESSED_COLUMNS,
    BanrepInflationConfig,
    PROCESSED_DIR,
    RAW_BANREP_DIR,
)
from src.io_utils import save_csv

logger = logging.getLogger("nairu_pipeline.banrep.inflation")


# ═══════════════════════════════════════════════════════════════════════
# 1. SESIÓN  (warm-up)
# ═══════════════════════════════════════════════════════════════════════


def create_session(
    config: BanrepInflationConfig = BANREP_INFLATION_CONFIG,
) -> requests.Session:
    """Crea una sesión HTTP con warm-up para SUAMECA.

    El backend de SUAMECA requiere una visita previa a la página
    pública para establecer las cookies de sesión.  Sin este paso,
    las llamadas al API devuelven 403.

    Returns
    -------
    requests.Session
        Sesión autenticada lista para consultar el API.
    """
    session = requests.Session()
    session.headers.update(config.http_headers)

    logger.info("Warm-up SUAMECA: %s", config.warmup_url)
    resp = session.get(config.warmup_url, timeout=config.timeout)
    resp.raise_for_status()
    logger.info("Warm-up OK: %d bytes", len(resp.content))

    return session


# ═══════════════════════════════════════════════════════════════════════
# 2. EXTRACCIÓN  (API REST)
# ═══════════════════════════════════════════════════════════════════════


def fetch_series(
    session: requests.Session,
    id_serie: int,
    tipo_dato: int,
    cant_datos: int = 2400,
    config: BanrepInflationConfig = BANREP_INFLATION_CONFIG,
) -> list[list]:
    """Consulta una serie individual del API de SUAMECA.

    Parameters
    ----------
    session : requests.Session
        Sesión con cookies de warm-up.
    id_serie : int
        Identificador de la serie en SUAMECA.
    tipo_dato : int
        Tipo de dato (9 = mensual, 18 = anual, etc.).
    cant_datos : int
        Cantidad máxima de observaciones a solicitar.

    Returns
    -------
    list[list]
        Lista de ``[epoch_ms, valor]`` tal como la devuelve el API.

    Raises
    ------
    ValueError
        Si la respuesta no tiene la estructura esperada.
    """
    params = {
        "idSerie": id_serie,
        "tipoDato": tipo_dato,
        "cantDatos": cant_datos,
    }

    logger.info(
        "Consultando serie %d (tipoDato=%d, cantDatos=%d)",
        id_serie, tipo_dato, cant_datos,
    )

    resp = session.get(
        config.endpoint_url,
        params=params,
        headers={"Referer": config.warmup_url},
        timeout=config.timeout,
    )
    resp.raise_for_status()

    payload = resp.json()

    # Validar estructura: lista con al menos un elemento que tenga "data"
    if not isinstance(payload, list) or len(payload) == 0:
        raise ValueError(
            f"Respuesta inesperada para serie {id_serie}: "
            f"se esperaba lista no vacía, se obtuvo {type(payload)}"
        )

    serie_obj = payload[0]
    if "data" not in serie_obj:
        raise ValueError(
            f"Respuesta para serie {id_serie} no contiene campo 'data'. "
            f"Claves disponibles: {list(serie_obj.keys())}"
        )

    data = serie_obj["data"]
    logger.info(
        "Serie %d: %d observaciones, nombre='%s'",
        id_serie, len(data), serie_obj.get("nombre", "?"),
    )
    return data


def fetch_all_series(
    session: requests.Session,
    config: BanrepInflationConfig = BANREP_INFLATION_CONFIG,
) -> dict[str, list[list]]:
    """Descarga todas las series definidas en ``config.series_map``.

    Returns
    -------
    dict[str, list[list]]
        Mapa ``{nombre_columna: [[epoch_ms, valor], ...]}``
    """
    result: dict[str, list[list]] = {}
    for col_name, spec in config.series_map.items():
        data = fetch_series(
            session,
            id_serie=spec["id"],
            tipo_dato=spec["tipo_dato"],
            cant_datos=config.cant_datos,
            config=config,
        )
        result[col_name] = data
    return result


# ═══════════════════════════════════════════════════════════════════════
# 3. PARSING
# ═══════════════════════════════════════════════════════════════════════


def epoch_ms_to_date(epoch_ms: int | float) -> date:
    """Convierte epoch en milisegundos a ``date`` de inicio de mes.

    SUAMECA devuelve fechas como end-of-month epoch (e.g. 2026-02-28).
    Las normalizamos al primer día del mes para consistencia.
    """
    dt = datetime.fromtimestamp(epoch_ms / 1000, tz=None)
    return date(dt.year, dt.month, 1)


def parse_series_data(
    raw_data: list[list],
    col_name: str,
) -> pd.DataFrame:
    """Convierte la lista ``[[epoch_ms, valor], ...]`` en DataFrame.

    Returns
    -------
    pd.DataFrame
        Columnas: ``date``, ``{col_name}``
    """
    records = []
    for item in raw_data:
        if len(item) < 2:
            continue
        epoch_ms, valor = item[0], item[1]
        if valor is None:
            continue
        d = epoch_ms_to_date(epoch_ms)
        records.append({"date": d, col_name: float(valor)})

    df = pd.DataFrame(records)
    if df.empty:
        return df

    # Eliminar duplicados de fecha (quedarse con el último valor)
    df = df.drop_duplicates(subset=["date"], keep="last")
    return df.sort_values("date").reset_index(drop=True)


def merge_series(
    all_raw: dict[str, list[list]],
) -> pd.DataFrame:
    """Une todas las series en un DataFrame mensual único.

    Realiza outer-merge por fecha para preservar todos los meses
    donde al menos una serie tenga dato.  La meta de inflación
    (Inf_Goal) viene ya expandida a meses desde SUAMECA, así que
    solo se necesita el merge sin forward-fill adicional.

    Returns
    -------
    pd.DataFrame
        Columnas: ``date``, ``year``, ``month`` + una columna por serie.
    """
    dfs: list[pd.DataFrame] = []
    for col_name, raw_data in all_raw.items():
        df_serie = parse_series_data(raw_data, col_name)
        if not df_serie.empty:
            dfs.append(df_serie)

    if not dfs:
        raise ValueError("No se obtuvieron datos de ninguna serie BANREP.")

    # Merge secuencial por fecha
    merged = dfs[0]
    for df_next in dfs[1:]:
        merged = pd.merge(merged, df_next, on="date", how="outer")

    merged = merged.sort_values("date").reset_index(drop=True)
    merged["date"] = pd.to_datetime(merged["date"])
    merged["year"] = merged["date"].dt.year
    merged["month"] = merged["date"].dt.month

    logger.info(
        "Series unidas: %d filas, rango: %s → %s",
        len(merged), merged["date"].min(), merged["date"].max(),
    )
    return merged


# ═══════════════════════════════════════════════════════════════════════
# TRANSFORMACIÓN
# ═══════════════════════════════════════════════════════════════════════


def clean_banrep_data(
    all_raw: dict[str, list[list]],
    config: BanrepInflationConfig = BANREP_INFLATION_CONFIG,
) -> pd.DataFrame:
    """Pipeline: parsea JSON → merge → esquema final."""
    df = merge_series(all_raw)

    df["source"] = "BANREP_SUAMECA"
    df["download_date"] = date.today().isoformat()

    # Asegurar tipos
    df["year"] = df["year"].astype(int)
    df["month"] = df["month"].astype(int)

    # Eliminar duplicados por fecha
    dupes_before = len(df)
    df = df.drop_duplicates(subset=["date"], keep="first").copy()
    dupes_dropped = dupes_before - len(df)
    if dupes_dropped > 0:
        logger.warning("Se eliminaron %d filas duplicadas por fecha", dupes_dropped)

    # Seleccionar y ordenar columnas
    series_cols = list(config.series_map.keys())
    output_cols = ["date", "year", "month"] + series_cols + ["source", "download_date"]

    # Asegurar que todas las columnas de serie existan
    for col in series_cols:
        if col not in df.columns:
            df[col] = None

    df = df[output_cols].sort_values("date").reset_index(drop=True)

    logger.info(
        "BANREP limpio: %d filas, rango: %s → %s",
        len(df), df["date"].min(), df["date"].max(),
    )
    return df


# ═══════════════════════════════════════════════════════════════════════
# CARGA
# ═══════════════════════════════════════════════════════════════════════


def save_raw_json(
    all_raw: dict[str, list[list]],
    output_dir: Path = RAW_BANREP_DIR,
    config: BanrepInflationConfig = BANREP_INFLATION_CONFIG,
) -> Path:
    """Guarda el JSON crudo para auditoría."""
    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / config.raw_json_filename
    path.write_text(json.dumps(all_raw, ensure_ascii=False, indent=2), encoding="utf-8")
    logger.info("JSON crudo guardado: %s", path)
    return path


def save_processed_data(
    df: pd.DataFrame,
    output_dir: Path = PROCESSED_DIR,
    config: BanrepInflationConfig = BANREP_INFLATION_CONFIG,
) -> Path:
    """Guarda el dataset de inflación procesado en disco."""
    output_path = output_dir / config.processed_filename
    save_csv(df, output_path)
    logger.info("Dataset procesado guardado: %s", output_path)
    return output_path


# ═══════════════════════════════════════════════════════════════════════
# ORQUESTACIÓN
# ═══════════════════════════════════════════════════════════════════════


def run_banrep_inflation_pipeline(
    config: BanrepInflationConfig = BANREP_INFLATION_CONFIG,
    output_dir: Path = PROCESSED_DIR,
    raw_dir: Path = RAW_BANREP_DIR,
) -> pd.DataFrame:
    """Pipeline completo: sesión → extracción → parsing → guardado."""
    # 1. Crear sesión
    session = create_session(config)

    # 2. Descargar todas las series
    all_raw = fetch_all_series(session, config)

    # 3. Guardar JSON crudo
    save_raw_json(all_raw, output_dir=raw_dir, config=config)

    # 4. Parsear y limpiar
    df = clean_banrep_data(all_raw, config)

    # 5. Guardar
    save_processed_data(df, output_dir=output_dir, config=config)

    return df
